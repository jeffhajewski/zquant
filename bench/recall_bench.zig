...
fn recall(a: std.mem.Allocator, dim: u32, bits: u6, n: usize, nq: usize, ks: []const usize, clustered: bool) ![]f64 {
    ...
    defer q.deinit();
    var ws = try zq.prod.Workspace.init(a, q);
    defer ws.deinit();
    var st = try zq.prod.QueryState.init(a, q);
    defer st.deinit();

    const corpus = try a.alloc(f32, n * dim);
    defer a.free(corpus);
    const codes = try a.alloc(u8, n * q.codeLen());
    defer a.free(codes);
    const sk = try a.alloc(u8, n * q.sketchLen());
    defer a.free(sk);
    const sc = try a.alloc(zq.prod.Scalars, n);
    defer a.free(sc);

    var prng = std.Random.DefaultPrng.init(7);
    const rnd = prng.random();
    const nc = 32;
    const centers = try a.alloc(f32, nc * dim);
    defer a.free(centers);
    ...
    const query = try a.alloc(f32, dim);
    defer a.free(query);
    const es = try a.alloc(f64, n);
    defer a.free(es);
    const as_ = try a.alloc(f64, n);
    defer a.free(as_);
    ...
    for (0..nq) |_| {
        ...
        std.mem.sort(u32, order, as_, struct {
            fn lt(s: []const f64, x: u32, y: u32) bool {
                return s[x] > s[y];
            }
        }.lt);
        for (ks, hits) |k, *h| {
            for (order[0..@min(k, n)]) |c| if (c == best) {
                h.* += 1;
                break;
            };
        }
